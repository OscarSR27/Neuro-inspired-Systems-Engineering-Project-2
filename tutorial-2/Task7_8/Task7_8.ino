...
// Constants
const float frequency = 0.5;  // Frequency in Hertz
const int amplitude = 300;    // Amplitude
...
void loop() {
  ...
  unsigned long time = micros() % period;  // Current time within the period
  float angle = 360.0 * ((float)time / period);  // Convert time to angle in degrees
  float sineValue = (amplitude * sin(radians(angle))) + 500;  // Calculate sine value
  
  // put your main code here, to run repeatedly:
  dxl_comm_result = packetHandler->write2ByteTxRx(portHandler, dxl_new_id, 30, int(sineValue), &dxl_error);delay(1);
